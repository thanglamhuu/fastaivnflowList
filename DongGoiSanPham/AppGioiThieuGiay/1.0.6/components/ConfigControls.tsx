    ...
    <div className="space-y-2 bg-white/[0.02] rounded-xl p-1 border border-white/5 shadow-inner">
      ...
      <div className="flex items-center justify-between gap-2 px-1">
        ...
        <select 
          ...
          className="bg-[#1c192b] border border-slate-800 rounded-lg px-2 py-1 text-[9px] focus:border-[#F31B17] outline-none cursor-pointer flex-1 text-right text-white"
        >
        ...
        <div className="flex gap-1">
          {THREAD_OPTIONS.map(opt => (
            <button 
              ...
              className={`w-8 py-1 text-[9px] font-black rounded-md border transition-all ${config.threads === opt ? 'bg-[#F31B17] border-red-400 text-white' : 'bg-slate-900 border-slate-800 text-slate-500'}`}
            >
              {opt}
            </button>
          ))}
        ...
        <div className="flex gap-1">
          {SPEEDS.map(opt => (
            <button 
              ...
              className={`px-2 py-1 text-[9px] font-black rounded-md border transition-all ${config.speed === opt ? 'bg-[#F31B17] border-red-400 text-white' : 'bg-slate-900 border-slate-800 text-slate-500'}`}
            >
              {opt}
            </button>
          ))}
        ...
        <div className="flex gap-1">
          {RESOLUTIONS.map(opt => (
            <button 
              ...
              className={`px-2 py-1 text-[9px] font-black rounded-md border transition-all ${config.resolution === opt ? 'bg-[#F31B17] border-red-400 text-white' : 'bg-slate-900 border-slate-800 text-slate-500'}`}
            >
              {opt}
            </button>
          ))}
