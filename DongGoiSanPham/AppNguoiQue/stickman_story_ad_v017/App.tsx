  ...
  return (
    <div className="flex h-screen w-screen bg-[#0e0e0e] text-white overflow-hidden font-sans">
      ...
      <div className="border-r border-white/10 flex flex-col justify-between p-3 w-[300px] h-full bg-[#0e0e0e] shrink-0">
        <div className="flex flex-col gap-6 overflow-y-auto dark-scrollbar pr-1">
          <div className="flex flex-col gap-3">
            <SectionLabel>Thông tin Sản phẩm (QC)</SectionLabel>
            <div className="p-3 rounded-xl border border-white/10 bg-white/5 space-y-3">
              <button onClick={handleSelectProductImage} className="w-full aspect-square rounded-lg border border-dashed border-white/20 hover:border-white/40 flex flex-col items-center justify-center gap-2 transition-colors overflow-hidden relative group">
                {config.productImage ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="text-[10px] text-white/30 font-bold uppercase">Ảnh sản phẩm</span>
                  </>
                )}
              </button>
              <input type="text" placeholder="Tên sản phẩm..." className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[11px] focus:outline-none" value={config.productName} onChange={(e) => setConfig(p => ({...p, productName: e.target.value}))} />
              <textarea placeholder="Mô tả..." className="w-full h-20 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[11px] focus:outline-none resize-none" value={config.productDesc} onChange={(e) => setConfig(p => ({...p, productDesc: e.target.value}))} />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="w-[160px]"><SegmentedToggle value={String(config.duration)} items={[{value: '4', label: '4s'}, {value: '6', label: '6s'}, {value: '8', label: '8s'}, {value: '10', label: '10s'}]} onChange={(v) => setConfig(p => ({ ...p, duration: Number(v) }))} /></div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 h-full overflow-y-auto p-6 paper-bg dark-scrollbar">
        ...
        {currentStep === 1 && (
          <div className="max-w-2xl mx-auto py-20 space-y-8 animate-dropdown">
            ...
            <div className="grid grid-cols-2 gap-2">
              {SUGGESTED_THEMES.slice(0, 4).map((t, i) => (
                <button key={i} onClick={() => generateScript(t)} className="text-left p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all text-sm text-white/70">{t}</button>
              ))}
            </div>
            <div className="relative mt-10">
              <textarea value={theme} onChange={(e) => setTheme(e.target.value)} placeholder="Nhập ý tưởng cốt truyện bằng Tiếng Việt..." className="w-full h-32 bg-white/5 border border-white/10 rounded-2xl p-4 text-sm focus:outline-none resize-none" />
              <div className="absolute bottom-3 right-3">
                <PillButton variant="solid" disabled={!theme.trim() || isProcessing} onClick={() => generateScript(theme)}>{isProcessing ? 'Đang soạn kịch bản...' : 'Bắt đầu kể chuyện'}</PillButton>
              </div>
            </div>
          </div>
        )}
